...
class Settings:
    ...
    detector_mode: str = os.getenv("DETECTOR_MODE", "person_hog")
    yolo_model: str = os.getenv("YOLO_MODEL", str(BASE_DIR / "yolo11n-pose.pt"))
    yolo_confidence: float = max(0.05, _float("YOLO_CONFIDENCE", 0.40))
    ...
    pose_keypoint_confidence: float = max(0.1, _float("POSE_KEYPOINT_CONFIDENCE", 0.30))
    ...
    person_process_interval: int = max(1, _int("PERSON_PROCESS_INTERVAL", 3))
    person_min_score: float = max(0.1, _float("PERSON_MIN_SCORE", 0.55))
    person_min_height: int = max(40, _int("PERSON_MIN_HEIGHT", 100))
    person_min_area_ratio: float = min(0.5, max(0.0, _float("PERSON_MIN_AREA_RATIO", 0.012)))
    person_duplicate_overlap: float = min(0.95, max(0.1, _float("PERSON_DUPLICATE_OVERLAP", 0.55)))
